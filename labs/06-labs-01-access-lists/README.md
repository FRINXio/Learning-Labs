# FRINX OpenDaylight & UniConfig - Access Lists

In this lab, you will learn to create, remove and place access-lists on devices using FRINX OpenDaylight (ODL) with UniConfig. 


## Creating an access list 

[Step 1: Create an ipv4 or ipv6 access list](1.md)

[Step 2: UniConfig API call to commit configuration](2.md)

[Step 3: UniConfig API call to show that access list was created](3.md)


## Deleting an access list

[Step 1: Delete access list](4.md)

[Step 2: UniConfig API call to commit configuration](5.md)

[Step 3: UniConfig API call to show that access list was deleted](6.md)


## Placing access list on an interface

[Step 1: Create access list ingress or egress on interface](7.md)

[Step 2: UniConfig API call to commit configuration](8.md)

[Step 3: UniConfig API call to show placed access list](9.md)


## Deleting access list from an interface

[Step 1: Delete access list ingress or egress from interface](10.md)

[Step 2: UniConfig API call to commit configuration](11.md)

[Step 3: UniConfig API call to show deleted access list](12.md)
